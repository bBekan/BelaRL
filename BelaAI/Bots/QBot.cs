...
namespace BelaAI
{
    internal class QBot : Player
    {
        ...
        private float Exploitation{ get; set; }
        private string? LastState { get; set; }
        private int GameNumber { get; set; }
        private bool Learn { get; set; }
        private double LearningRate { get; set; } = 0.001;
        private double Discount { get; set; } = 0.1;
        ...
        private double BestPossibleNext(Card toPlay, int maxCount, List<Card> remaining, Card currentWinning)
        {
            if (maxCount == 0)
            {
                var bestKey = KeyFromState(currentWinning, toPlay, Played);
                return States[bestKey];
            }
